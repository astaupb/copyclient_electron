...
Section ""
	...
	File "dist\AStA Copyclient 0.0.12.exe"
	File "license.txt"
	File /r "fakeprinter\windows"
	Rename "$INSTDIR\windows" "$INSTDIR\fakeprinter"
	WriteUninstaller "$INSTDIR\uninstall.exe"
	CreateDirectory "$SMPROGRAMS\AStA Uni Paderborn"
	CreateShortCut "$SMPROGRAMS\AStA Uni Paderborn\AStA Copyclient.lnk" "$INSTDIR\AStA Copyclient 0.0.12.exe" ""
	CreateShortCut "$SMPROGRAMS\AStA Uni Paderborn\Deinstallieren.lnk" "$INSTDIR\uninstall.exe" ""
	CreateShortCut "$DESKTOP\AStA Copyclient.lnk" "$INSTDIR\AStA Copyclient 0.0.12.exe" ""
	CreateShortCut "$SMSTARTUP\AStA Copyclient starten.lnk" "$INSTDIR\AStA Copyclient 0.0.12.exe" ""
...
Section "Uninstall"
	...
	Exec '"$WINDIR\System32\taskkill.exe" /f /im AStA Copyclient 0.0.12.exe'
	Delete "$INSTDIR\uninstall.exe"
	Delete "$INSTDIR\AStA Copyclient 0.0.12.exe"
