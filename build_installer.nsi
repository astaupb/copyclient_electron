...
Section ""
	SetOutPath "$INSTDIR"
	File "dist\AStA Copyclient 0.0.10.exe"
	File "license.txt"
	File /r "fakeprinter\windows"
	Rename "$INSTDIR\windows" "$INSTDIR\fakeprinter"
	WriteUninstaller "$INSTDIR\uninstall.exe"
	CreateDirectory "$SMPROGRAMS\AStA Uni Paderborn"
	CreateShortCut "$SMPROGRAMS\AStA Uni Paderborn\AStA Copyclient.lnk" "$INSTDIR\AStA Copyclient 0.0.10.exe" ""
	CreateShortCut "$SMPROGRAMS\AStA Uni Paderborn\Deinstallieren.lnk" "$INSTDIR\uninstall.exe" ""
	CreateShortCut "$DESKTOP\AStA Copyclient.lnk" "$INSTDIR\AStA Copyclient 0.0.10.exe" ""
	CreateShortCut "$SMSTARTUP\AStA Copyclient.lnk" "$INSTDIR\AStA Copyclient 0.0.10.exe" ""
...
Section "Uninstall"
	...
	Exec '"$WINDIR\System32\taskkill.exe" /f /im AStA Copyclient 0.0.10.exe'
	Delete "$INSTDIR\uninstall.exe"
	Delete "$INSTDIR\AStA Copyclient 0.0.10.exe"
